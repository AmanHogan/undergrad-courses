...
void process_image(unsigned char image[DIM][DIM], int size[2], unsigned char proc_img[DIM][DIM])
{
    ...
    for (int x = 0; x < size[0]; x++) 
    {
        for (int y = 0; y < size[1]; y++) 
        {
            ...
            proc_img[x][y] = (unsigned char) gradient_magnitude;
        }
    }
 ...
 * @return None
*/
void template_matching_process_image(unsigned char image[DIM][DIM], int size[2], unsigned char proc_img[DIM][DIM])
{

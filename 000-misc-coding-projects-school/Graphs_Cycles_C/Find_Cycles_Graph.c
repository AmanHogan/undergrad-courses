...
// Special Case: no course has any prerequisite
//Special Case because usually a student would need a prerquisite to enroll in a course//
#define MAX_FILENAME 1001
#define MAX_CHAR 31
...
int find_node_total(char* filename, char*** str_nodes);
int** alloc2d(int R, int C);
graphPT newGraph(int nodes);
void free_memory_graph(graphPT g);
...
int locate_ind(char** str_nodes, char* token, int N);
...
int locate_ind(char** str_nodes, char* token, int N)
{
	...
	{
		if( strcmp(str_nodes[index], token) == 0 )
		{
			return index;
		}
	}
...
int find_node_total(char* filename, char*** str_nodes)
{
	...
	*str_nodes = (char**) malloc(sizeof(char*) * node_total);
	fseek(fp, 0 , SEEK_SET);
	for(int i = 0; i < node_total; i++)
	{
		(*str_nodes)[i] = (char*) malloc(sizeof(char) * MAX_CHAR);
		strcpy( (*str_nodes)[i], "" );
	}
	...
	while(!feof(fp))
	{
		fgets(fileline, MAX_FILENAME - 1, fp);
		token = strtok(fileline, " \n");
		strcpy( (*str_nodes)[index], token );
		index++;
	}
	fclose(fp);
	return node_total;
...
void Depth_First_Search(graphPT* g, Stack* s, color* black, color* white, color* grey,  int** d, int** f, int* E, int* t, int current, int* cycle)
{
	...
	*t = *t + 1;
	...
	grey->array_of_edges[current] = 1;
	white->array_of_edges[current] = 0;
	...
	*t = *t + 1;
	(*f)[current] = *t;
	grey->array_of_edges[current] = 0;
	black->array_of_edges[current] = 1;
